...
namespace QLCHS.Controllers
{
    ...
    public class LoginController : ControllerBase
    {
        ...
        public async Task<IActionResult> SignIn([FromBody] SignInRequest request)
        {
            ...
            var user = await _context.Customers.FirstOrDefaultAsync(x => x.Phone == request.Phone);

            // Kiểm tra thông tin người dùng
            if (user == null || EncDscPassword.DecryptPassword(user.Password) != request.Password)
            {
            ...
            var claims = new[]
            {
        new Claim(ClaimTypes.Name, user.Id.ToString()), // Sử dụng ToString() để đảm bảo kiểu dữ liệu
